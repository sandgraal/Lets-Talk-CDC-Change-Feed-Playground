...
| `ff_schema_demo` | Core duo | Drives the schema change demo with add/drop column flows and schema version badges. | Enabled by default in `index.html` so the comparator walkthrough + tests stay in sync. | Remains on now that the scenario smoke is covered; toggle only when debugging schema issues. |
| `ff_multitable` | Core duo | Exposes multi-table + transactional scenarios with apply-on-commit coordination. | Enabled by default in `index.html` so transaction coverage + tutorials stay green. | Keep enabled for day-to-day demos; only disable locally when isolating non-transaction bugs. |
